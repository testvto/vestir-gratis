...
class RelatedPostsThumbnails {
	...
	function get_thumbnails( $show_top = false ) { // Retrieve Related Posts HTML for output
		...
		if ($relpoststh_output_style == 'list') {
			...
			//$output .= '<div style="clear: both"></div><div style="border: 0pt none ; margin: 0pt; padding: 0pt;">'; //vtrio
			$output .= '<div style="border: 0pt none ; margin: 0pt; padding: 0pt; padding-left:0px">';
		foreach( $posts as $post ) {
			...
			if ($relpoststh_output_style == 'list') {
				...
				$output .= '<div class="game3" onmouseout="this.className=\''.game3.'\'" onmouseover="this.className=\''.game4.'\'"  style="width: ' . $width . 'px; height: ' . $height . 'px;">';
				$output .= '<a title="'.get_post_meta($post->ID, "description", true).'" href="' . get_permalink( $post->ID ) . '" >';	
				$output .= '<div style=" background: transparent url(' . $url . ') no-repeat scroll 0% 0%; -moz-background-clip: border; -moz-background-origin: padding; -moz-background-inline-policy: continuous; width: ' . $width . 'px; height: ' . $height . 'px;"> ';
				$output .= '</div>';
				$output .= '</a>';
				$output .= '<h3><a title="'.get_post_meta($post->ID, "description", true).'" href="' . get_permalink( $post->ID ) . '" rel="bookmark" title="Jugar">JUGAR</a></h3>';	
				$output .= '<h4><a title="'.get_post_meta($post->ID, "description", true).'" href="' . get_permalink( $post->ID ) . '" rel="bookmark" title="Jugar">' . $title . $excerpt . '</a></h4>';
				$output .= '</div>';
